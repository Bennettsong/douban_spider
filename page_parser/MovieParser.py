...
class MovieParser:
    ...
    def __set_bs_soup(self):
        ...
        self.__soup = BeautifulSoup(self.__html_doc, 'lxml')
    ...
    def __get_vote_num(self):
        try:
            info = self.__soup.find('span', {'property': 'v:votes'})
            self.__movie['vote_num'] = info.text
        except:
            pass

    def __get_rating_per_stars(self):
        try:
            info = self.__soup.find_all('span', {'class': 'rating_per'})
            self.__movie['rating_per_stars5'] = info[0].text
            self.__movie['rating_per_stars4'] = info[1].text
            self.__movie['rating_per_stars3'] = info[2].text
            self.__movie['rating_per_stars2'] = info[3].text
            self.__movie['rating_per_stars1'] = info[4].text
        except:
            pass

    def __get_comment_num(self, response):
        try:
            result = response.xpath('//*[@id="comments-section"]/div[1]/h2/span/a/text()')[0].strip()
            self.__movie['comment_num'] = re.search('(\d+)', result).group()
        except:
            pass
    ...
    def __get_recommend(self):
        try:
            ...
            result = ''
            for item in info: result += item.find('a').text + ','
            self.__movie['recommendMovie'] = MovieParser.__trim_last_comma(result)
        except:
            pass
    ...
    def extract_movie_info(self, response):
        ...
        :return: None|dict
        """
        self.set_html_doc(response.text)
        ...
        self.__get_vote_num()
        self.__get_rating_per_stars()
        self.__get_comment_num(r)
